...
def practice_problem4c(points):
    ...
    ####################################################################
    for p in points:
        if is_prime(p.x) and is_prime(p.y):
            temp = p.x
            p.x = p.y
            p.y = temp
            return p
...
def practice_problem4d(sequence):
    ...
    if len(sequence) < 2:
        return 0
    for k in range(len(sequence) - 1):
        current_value = sequence[k]
        next_value = sequence[k + 1]
        if current_value != next_value and is_prime(current_value) and is_prime(next_value):
            total += current_value
    return total
